...
//=============================================================================
 ...
 async function f_response() {
   try {
         ...
         //let response = await fetch('http://alphagameset.xyz/1_ArenaFjs/_SQLBase/get_arena_f_js_SQL_base.php/?name=requests_arena_f_js');
         let response = await fetch('http://alphagameset.xyz/0_0_SQL_base/CountRequestsGet.php/?name=requests_arena_f_js');
         if (response.ok) { // если HTTP-статус в диапазоне 200-299
           ArenaFjs_R.number_users = await response.text();
           ArenaFjs_R.get_text_number_users.value = "Запросов " + ArenaFjs_R.number_users;
         } else {
...
ArenaFjs_R.frameG = 0;
 //loop>------------------------------------------------------------------------
  ...
  var w = 0;// для теста загрузки картинок
  ...
  ArenaFjs_R.timerId = setTimeout( function tick(){
        ...
        if( (ArenaFjs_R.window_load_end == 1) && (ArenaFjs_R.startGame == 0) ){
          ...
          // для теста загрузки картинок
           if (w == 0) {
             w = 1;
             SpritesFighter_R.loadAllSprite_w();
           }
        };
